...
class Element:
    ...
    # Набуті кут, кривизна та скрут (з урахуванням початкових)
    dTh: vector
    K: float
    T: float
    ...
    def copy(self) -> 'Element':
        """ Створює копію елементу """
        t, xi, eta = self.mat
        new = Element(self.p.copy(), self.s, t.copy(), xi.copy(), self.K0, self.T0, self.EI, self.GJ)
        new.mat = self.mat.copy()
        new.nat = self.nat.copy()
        new.dTh = self.dTh
        new.K = self.K
        new.T = self.T
        return new
    ...
    def join_after(self, prev: 'Element'):
        e_p = prev
        e_n = self
